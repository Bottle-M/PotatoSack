...
7. 为了控制记录文件的规模，对每个备份目录分别生成对应的记录文件来进行记录。
...
这种方式下要先模拟压缩备份文件一遍以计算压缩文件大小，然后再压缩一遍，边压缩边上传到 OneDrive。
...
在上一部分我考虑了文件读写引发的完整性问题，但还存在一种情况：  

程序首先会扫描备份目录下的所有文件，组成一个列表，然后再根据这种列表进行压缩工作。  

如果在扫描文件至开始压缩文件这段时间内，有文件被删除了呢？这样就可能出现下列两种情况：  

1. 在开始读取某个文件**前**，这个文件已经被删除（文件已不存在）。
2. 在读取某个文件**时**，这个文件被尝试删除。  
...
因为打开文件是通过文件描述符与文件系统进行关联的，每个文件都有一个 `inode`，而文件描述符通过文件打开表与 `inode` 进行关联。删除只是删除了文件的目录项，移除了目录项和 `inode` 的关联，而文件描述符和 `inode` 的关联仍然存在。  

因为还有关联，这个时候 `inode` 不会被移除，只不过无法通过文件名在目录中找到相应文件了。当文件描述符被关闭后（在这里是文件读取完毕），`inode` 才将被移除。

**解决方案**：如果是第一种情况，忽略这个文件即可；而对于第二种情况，顺其自然，可能会抛出 `IOException`。
