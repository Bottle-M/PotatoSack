...
3. 如果这一组备份是当天的（比如 `020240104000002` 对应2024年1月4日），则从目录中下载 `backup.json`、`_备份路径标识.json` 到本地。
...
1. 检查本地（插件目录）是否有备份记录文件 `backup.json` 以及`_备份路径标识.json`
   ，如果没有则会[从云端尝试拉取](#从云端拉取备份记录文件)对应的备份记录文件 `backup.json` 和 `_备份路径标识.json` 。
    - 一个备份记录文件对应**一组备份**
    - 本地只存储当前一组备份的记录
