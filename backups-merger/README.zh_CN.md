...
> **该用哪个版本？** 从 PotatoSack 3.0.0 起，增量备份里的 `.mca` 只存储发生变动的区块（即看上去是 `.mca`，实际上是 PotatoSack 的 `PSMCA` delta 格式），
> 因此恢复这类备份组需要使用 **BackupsMerger 1.1.0 或更新版本**。旧版本只会把 `.mca` 条目原样解压，
> 结果会在存档目录里留下 Minecraft 读不了的区域文件。旧版 PotatoSack 产生的备份组（增量包里是完整的
> `.mca` 文件）仍然可以正常合并。
...
| 存放完整区域文件的 `.mca`（`full.zip` 中总是如此；旧版本或生产端退化为原样打包的文件也是如此） | 原样解压、覆盖目标文件 |
| 以 `PSMCA\0` 开头的 `.mca`（3.0.0 增量 delta 格式，只含变动区块）         | 读取前面备份已经恢复出来的区域文件作为基线，把 delta 应用进去，重新生成完整的 Anvil 区域文件后再覆盖 |
| `deleted.files`                                          | 该增量包的其余条目都处理完之后，按清单逐行删除文件；清单本身随后被删除 |

* delta 里没有记录的区块沿用此前状态；同一个区域文件可以连续应用多份 delta。
* delta 中扇区数为 `0` 的区块会从区域文件中删除。
* delta 不携带新的区块时间戳，因此区域文件头部（后 4 KiB）的时间戳表**沿用基线**。
* `.mcc` 按普通文件处理：`.mca` 里对于 `.mcc` 这种外置区块的存根会随区块数据一起复制。
* 每个 `.mca` 都是先写到临时文件、再由临时文件原子替换目标文件，因此失败不会留下写了一半的区域文件。
  临时文件都放在程序自己的工作目录里，退出时会整体删除。

## 出错排查

| 提示信息 | 含义                                                               |
| --- |------------------------------------------------------------------|
| `Full backup contains an incremental (PSMCA) region entry: …` | `full.zip` 里出现了 delta 形式的 `.mca`。全量备份必须原样存储区域文件，说明这个备份组不一致，请重新下载。 |
| `Incremental region entry … is a PSMCA delta, but there is no baseline region file at …` | 该增量需要用到前面备份恢复出的区域文件作为基线。请确认选对了备份组，且 `full.zip` 中包含这个区域文件。        |
