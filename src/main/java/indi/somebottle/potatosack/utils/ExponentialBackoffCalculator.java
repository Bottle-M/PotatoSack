...
public class ExponentialBackoffCalculator {
     ...
     */
    public ExponentialBackoffCalculator(long baseBackoff) {
     ...
     */
    public void backoff() {
        nextBackoffTime = Math.min(nextBackoffTime * 2, Long.MAX_VALUE);
    }

    /**
     * 获取下次退避时间，此方法不会更新退避时间
     *
     * @param upperBound 退避时间上限（某个单位）
     * @return 下次退避时间（某个单位）
     */
    public long getNextBackoffTime(long upperBound) {
        return Math.min(nextBackoffTime, upperBound);
    }

    public void reset() {
        nextBackoffTime = baseBackoff;
    }
}
